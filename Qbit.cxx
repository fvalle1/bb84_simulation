...
Qbit &Qbit::operator=(const Qbit &source) {
  ...
  if(this == &source) return *this;               // ??? non ho ben capito queste 4 righe 
  this->~Qbit();
...
void Qbit::MeasureState(basis b) {
  if (Qbit::DEBUG) printf("\nMeasuring qbit\n");                                             // setto la base
                                                                                             // vedo se il qbit è logico --> misuro e faccio l'eventuale correzione
  //Qbit collapse to the basis used for measuring                                            // preparo il nuovo theta
  fBase = b;
